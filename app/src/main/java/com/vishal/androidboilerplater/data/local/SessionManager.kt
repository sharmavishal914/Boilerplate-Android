package com.vishal.androidboilerplater.data.local

import android.content.Context
import android.content.SharedPreferences

class SessionManager constructor(context: Context) {
    val mSharedPreferences: SharedPreferences = context.getSharedPreferences("Pref", Context.MODE_PRIVATE)
    ...
    inline fun <reified T : Any> get(key: String, defaultValue: T? = null): T? {
        return when (T::class) {
            ...
            Boolean::class -> mSharedPreferences.getBoolean(
                key, defaultValue as? Boolean
                    ?: false
            ) as T?
            Float::class -> mSharedPreferences.getFloat(
                key,
